...
const Team = () => {
  return (
    <div id='team' className='relative'>
      <Lottie className='absolute pointer-events-none right-0 -top-2/3 max-2xl:-top-1/3 max-2xl:max-w-96 max-xl:max-w-72 max-md:max-w-[198px] max-md:top-1/4 -z-[1] max-md:hidden' animationData={rightLottie}/>
        ...
        <div className="flex items- justify-center gap-6 flex-wrap">
          {TEAM_LIST.map((obj, i) => (
            <div key={i} className='bg-gradient-to-r from-[#B62AFF80] to-[#FF0DF280] p-[1px] rounded-[25px] hover:scale-y-[1.07] group duration-300 ease-linear max-xl:max-w-[380px] hover:translate-y-[-20px] max-xl:hover:translate-y-0'>
              <div className="bg-smoke-white px-14 pb-5 rounded-[25px] h-full w-full">
                <img className='-translate-y-[36px] w-full -mb-4 group-hover:scale-110 group-hover:translate-y-[-50px] duration-300 ease-linear pointer-events-none max-lg:group-hover:translate-y-[-20px] max-lg:max-w-[200px]' src={obj.image} alt="team member" />
