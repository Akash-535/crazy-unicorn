            ...
            <div className="bg-gradient-to-r from-[#B62AFF80] to-[#FF0DF280] p-[1px] rounded-[10px] max-w-[521px] mx-auto">
              <div className="w-full flex bg-white max-w-[521px] mx-auto justify-between px-2.5 pt-[11.35px] pb-2 rounded-[10px]">
                <div className="flex items-center sm:justify-center gap-2">
                  <EmailIcon />
                  <input className="bg-transparent sm:w-[250px] max-sm:pr-2 outline-none w-full text-base text-black placeholder:text-black placeholder:opacity-70 font-comic" type="email" placeholder="Your email" name="email" onChange={(e) => setValue({ ...value, email: e.target.value })} value={value.email} />
                </div>
                <div className='bg-gradient-to-l from-pink-dark to-neon-pink max-w-[234px] rounded-xl'>
                  <button type='submit' className='bg-white sm:text-[25px] text-xl border-[1px] border-black rounded-xl -translate-x-[3.5px] -translate-y-[3.5px] hover:translate-x-0 hover:translate-y-0 duration-300 ease-linear py-[10px] px-7'><span className='bg-gradient-to-r from-pink-dark to-neon-pink bg-clip-text text-transparent font-normal '>Subscribe</span></button>
                </div>
              </div>
            </div>
            <p className="text-red-500 pl-5 font-semibold max-w-[521px] mx-auto max-sm:hidden">{error && !value.email ? 'Field is required' : ''}</p>
          </form>
        </div>
      </div>
    </div>
