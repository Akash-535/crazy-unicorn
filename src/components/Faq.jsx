...
const Faq = () => {
  ...
  return (
    <div id='faq' className="relative">
      <Lottie className='absolute pointer-events-none right-0 top-20 max-2xl:max-w-96 max-xl:max-w-72 max-md:max-w-[198px] max-md:top-1/4 -z-[1] max-md:hidden' animationData={rightLottie}/>
      <Lottie className='absolute pointer-events-none left-0 -top-1/4 max-2xl:max-w-60 max-xl:max-w-48 max-md:max-w-[198px] max-md:-bottom-10 -z-[1] max-md:hidden' animationData={leftLottie}/>
      <div className="container xl:pb-[132px] lg:pb-24 pb-16 mx-auto max-xl:px-5 xl:pt-[103px] md:pt-16 pt-12">
        <div className='relative text-center max-w-[143px] mx-auto'>
          <img className='pointer-events-none max-w-[118px] absolute -top-14 max-lg:-top-14 -right-11 max-lg:-right-10 max-sm:-right-4 max-md:-top-12 max-md:max-w-[90px]' src="./assets/images/webp/little-horse-image.webp" alt="little horse" />
          ...
          {FAQ_DATA_LIST.map((item, index) => (
            <div className={`rounded-xl`}>
              <div key={index} className={`rounded-xl ! border-none gap-4 transition-all ease-linear duration-500 ${active === index ? '!border-none' : ''}`}>
                <button onClick={() => toggle(index)} className={`flex w-full max-lg:gap-12 uppercase justify-between max-sm:gap-4 bg-transparent  text-left font-medium lg:text-custom-4xl sm:text-3xl text-lg`}>
                  {item.heading}
